    ...
    public function import(bool $dryRun = false): array
    {
        ...
        foreach ($rows as $priority => $row) {
            ...
            $data = [
                ...
                'priority' => $priority,
                'scope_group_ids' => Mage::helper('core')->jsonEncode($this->csvInts((string) ($row['cust_groups'] ?? ''))),
                'scope_store_ids' => Mage::helper('core')->jsonEncode($this->csvInts((string) ($row['stores'] ?? ''))),
                'scope_country_codes' => Mage::helper('core')->jsonEncode([]),
                'scope_category_ids' => Mage::helper('core')->jsonEncode($this->csvInts((string) ($row['categories'] ?? ''))),
